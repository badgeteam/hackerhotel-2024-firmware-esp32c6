#pragma once

typedef enum _screen {
    screen_scrambled,
    screen_billboard,
    screen_nametag,
    screen_library,

    screen_hangman,
    screen_repertoire,
    screen_pointclick,
    screen_settings,
    screen_battleship,

    screen_credits,
    screen_settings_nickname,
    screen_settings_wifi,
    screen_morse,
    screen_shades,

    screen_welcome,
    screen_template,
    screen_mascots,
    screen_home,

    screen_BS_splash,
    screen_BS_placeships,
    screen_BS_battle,
    screen_BS_victory,

    screen_HM_splash,
    screen_HM_playing,
    screen_HM_victory,

    screen_PC_dock1,
    screen_PC_dock2,
    screen_PC_land1,
    screen_PC_dune1,
    screen_PC_dune2,
    screen_PC_town1,
    screen_PC_town2,
    screen_PC_road1,
    screen_PC_dune3,
    screen_PC_lighthouse1,
    screen_PC_lighthouse2,

    screen_PC_postoffice_closeup_d,
    screen_PC_shop_closeup,
    screen_PC_shop_closeup_d,
    screen_PC_shop_closeup_da,
    screen_PC_road1_e_jorge_d,

    screen_PC_dock1_w_d,
    screen_PC_dune2_n_d,
    screen_PC_dune3_n_gc_d,
    screen_PC_library_closeup,
    screen_PC_library_closeup_d1,
    screen_PC_lighthouse1_n_gc_d_key,
    screen_PC_lighthouse1_n_gc_d1,
    screen_PC_lighthouse2_n_go,
    screen_PC_messageboard_closeup,
    screen_PC_road1_e_jorge,
    screen_PC_town1_n_d1,
    screen_PC_town1_e_d1,
    screen_PC_town1_s_d1_boat,
    screen_PC_town1_s_d1_messageboard,
    screen_PC_end

} screen_t;
