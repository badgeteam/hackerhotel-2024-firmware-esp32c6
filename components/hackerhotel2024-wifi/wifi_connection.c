...
#define WIFI_SORT_ERRCHECK(err)                                                                                        \
    do {                                                                                                               \
        int res = (err);                                                                                               \
        if (res) {                                                                                                     \
            ESP_LOGE(TAG, "WiFi connection error: %s", esp_err_to_name(res));                                          \
            goto error;                                                                                                \
        }                                                                                                              \
    } while (0)
...
void wifi_connect_async(const char* aSsid, const char* aPassword, wifi_auth_mode_t aAuthmode, uint8_t aRetryMax) {
    ...
    memcpy(
        ...
        aPassword,
        strnlen(aPassword, 64)
    );  // Target does NOT have to be NULL terminated
...
void wifi_connect_ent_async(
    ...
    // Set WPA2 ENT config.
    WIFI_SORT_ERRCHECK(esp_eap_client_set_identity((const uint8_t*)aAnonIdent, strlen(aAnonIdent)));
    WIFI_SORT_ERRCHECK(esp_eap_client_set_username((const uint8_t*)aIdent, strlen(aIdent)));
    WIFI_SORT_ERRCHECK(esp_eap_client_set_password((const uint8_t*)aPassword, strlen(aPassword)));
    WIFI_SORT_ERRCHECK(esp_eap_client_set_ttls_phase2_method(phase2));
    // Enable enterprise auth.
    WIFI_SORT_ERRCHECK(esp_wifi_sta_enterprise_enable());
    // Disable 11b as NOC asked.
...
wifi_strength_t wifi_rssi_to_strength(int8_t rssi) {
    if (rssi > WIFI_THRESH_VERY_GOOD)
        return WIFI_STRENGTH_VERY_GOOD;
    else if (rssi > WIFI_THRESH_GOOD)
        return WIFI_STRENGTH_GOOD;
    else if (rssi > WIFI_THRESH_BAD)
        return WIFI_STRENGTH_BAD;
    else
        return WIFI_STRENGTH_VERY_BAD;
}
