...
static void display_test_state(
    const char*               text,
    const char*               ssid,
    const char*               password,
    ...
    pax_draw_text(pax_buffer, 0xFF000000, font, 18, 5, pax_buffer->height - 3 * 18, text);
    if (buttons)
        pax_draw_text(pax_buffer, 0xFF000000, font, 18, 5, pax_buffer->height - 18, "🅰 test 🅱 back");
    bsp_display_flush();
}

void wifi_connection_test() {
    QueueHandle_t button_queue = bsp_get_button_queue();
    ...
    while (!quit) {
        ...
        esp_http_client_config_t config = {
            .url                 = "https://mch2022.ota.bodge.team/test.bin",
